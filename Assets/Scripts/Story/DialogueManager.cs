...
public class DialogueManager : MonoBehaviour
{
    ...
    void UpdateDialogue()
    {
        ...
        else if (action == 3) dialogueText.text = $"Now we have {PlaManager.Instance.PlaBtnsCount-1} types of Pla and a rock. \nWhat does each Pla do? \nYou can point your mouse at each Pla.";
        else if (action == 4) dialogueText.text = $"Here are shortcuts:\nNum 1-{PlaManager.Instance.PlaBtnsCount} to select Pla\n-/+ to speed up or slow down the game speed\nSpace to hide the Pla panel on the right.\n";
        else if (action == 5) dialogueText.text = "That is all you need to know.\nOur lives depend on you now.";
